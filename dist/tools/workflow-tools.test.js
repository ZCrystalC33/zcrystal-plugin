...
describe('Workflow Tools', () => {
    ...
    it('should register zcrystal_workflow_create tool', async () => {
        ...
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_workflow_create')).toBe(true);
    });
    it('should register zcrystal_workflow_get tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_workflow_get')).toBe(true);
    });
    it('should register zcrystal_workflow_stats tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_workflow_stats')).toBe(true);
    });
    it('should register zcrystal_workflow_pause tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_workflow_pause')).toBe(true);
    });
    it('should register zcrystal_workflow_resume tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_workflow_resume')).toBe(true);
    });
    it('should register zcrystal_workflow_cancel tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_workflow_cancel')).toBe(true);
    });
    // Adapter Tools
    it('should register zcrystal_adapter_scan_openclaw tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_adapter_scan_openclaw')).toBe(true);
    });
    it('should register zcrystal_adapter_scan_zcrystal tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_adapter_scan_zcrystal')).toBe(true);
    });
    it('should register zcrystal_adapter_import tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_adapter_import')).toBe(true);
    });
    it('should register zcrystal_adapter_export tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_adapter_export')).toBe(true);
    });
    it('should register zcrystal_adapter_sync tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_adapter_sync')).toBe(true);
    });
    // Replay Tools
    it('should register zcrystal_replay_save tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_replay_save')).toBe(true);
    });
    it('should register zcrystal_replay_get tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_replay_get')).toBe(true);
    });
    it('should register zcrystal_replay_list tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_replay_list')).toBe(true);
    });
    it('should register zcrystal_replay_stats tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_replay_stats')).toBe(true);
    });
    it('should register zcrystal_replay_rollback tool', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_replay_rollback')).toBe(true);
    });
    ...
    it('should execute zcrystal_workflow_create', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        const tool = mockApi.getTools().find((t) => t.name === 'zcrystal_workflow_create');
    ...
    it('should execute zcrystal_workflow_stats', async () => {
        const { registerWorkflowTools } = await import('./workflow-tools.js');
        registerWorkflowTools(mockApi, mockState);
        const tool = mockApi.getTools().find((t) => t.name === 'zcrystal_workflow_stats');
