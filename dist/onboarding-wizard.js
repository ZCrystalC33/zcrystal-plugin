...
export class OnboardingWizard {
    ...
    initSteps() {
        this.steps = [
            ...
            },
            {
                ...
                validate: async () => {
                    try {
                        const resp = await fetch('http://localhost:18789/tools/list');
                        if (resp.ok) {
                            const data = await resp.json();
                            const hasFts5 = data.tools?.some((t) => t.name === 'fts5_search');
                            return hasFts5
    ...
    async getSystemSummary() {
        const lines = [];
        try {
            const gw = await fetch('http://localhost:18789/health');
            const gwData = await gw.json();
            lines.push(`  Gateway: ${gwData.ok ? '✅' : '❌'} ${gwData.status}`);
            const hc = await fetch('http://localhost:8000/health');
