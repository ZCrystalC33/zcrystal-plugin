...
import { HonchoClient, createHonchoClient } from './honcho-client.js';
import { SkillManager, createSkillManager } from './skill-manager.js';
import { SelfEvolutionEngine, createSelfEvolutionEngine } from './self-evolution.js';
import type { ExecutionTrace, ZCrystalConfig } from './types.js';
...
export default definePluginEntry({
  ...
  register(api: OpenClawPluginApi) {
    const config = api.config as ZCrystalConfig;
    ...
    api.registerTool({
      ...
      async execute(_id, params) {
        ...
        const results = await state.honcho.search(
          params.peer || 'user',
          params.query,
          params.limit || 10
        );
    ...
    api.registerTool({
      ...
      async execute(_id, params) {
        ...
        const response = await state.honcho.ask(
          'user',
          params.question,
          params.depth || 'quick'
        );
        ...
        const text = history.length === 0
          ...
          : `Evolution history: ${history.length} runs\n` +
            history.slice(-3).map(h => 
              `- ${h.target}: ${h.candidates.length} candidates, best: ${h.bestCandidate?.score.toFixed(2) || 'N/A'}`
            ).join('\n');
    ...
    api.registerTool({
      ...
      async execute(_id, params) {
        ...
        if (params.slug) {
          ...
          return okResult(
            `Evolved ${skill.name}: ${result.candidates.length} candidates, best: ${result.bestCandidate?.score.toFixed(2) || 'N/A'}`,
            { skill: params.slug, candidates: result.candidates.length }
          );
        } else {
    ...
    api.registerHook('after_tool_call', async (event) => {
      ...
      const { toolName, input, output, success, duration } = event as {
        toolName?: string;
        input?: unknown;
        output?: unknown;
        success?: boolean;
    ...
    api.registerHook('before_prompt_build', async (event) => {
      ...
      const ctx = event as { systemPrompt?: string };
