...
export default definePluginEntry({
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerTool({
      ...
      async execute(_id, params) {
        if (!state || !state.evolutionScheduler) return errResult('Plugin not initialized or scheduler unavailable');
        if (params.action === 'start') {
          ...
          const running = (state.evolutionScheduler as unknown as { running?: boolean }).running;
