...
describe('Proactive Tools', () => {
  ...
  it('should register zcrystal_correction_add', async () => {
    ...
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_correction_add')).toBe(true);
  });

  it('should register zcrystal_correction_list', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_correction_list')).toBe(true);
  });

  it('should register zcrystal_memory_add', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_memory_add')).toBe(true);
  });

  it('should register zcrystal_memory_get', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_memory_get')).toBe(true);
  });

  it('should register zcrystal_pattern_add', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_pattern_add')).toBe(true);
  });

  it('should register zcrystal_pattern_list', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_pattern_list')).toBe(true);
  });

  it('should register zcrystal_session_set', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_session_set')).toBe(true);
  });

  it('should register zcrystal_session_get', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_session_get')).toBe(true);
  });

  it('should register zcrystal_session_clear', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_session_clear')).toBe(true);
  });

  it('should register zcrystal_proactive_check', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_proactive_check')).toBe(true);
  });

  it('should register zcrystal_proactive_suggest', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_proactive_suggest')).toBe(true);
  });

  it('should register zcrystal_proactive_log', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_proactive_log')).toBe(true);
  });

  it('should register zcrystal_proactive_recent', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_proactive_recent')).toBe(true);
  });

  it('should register zcrystal_log_action', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_log_action')).toBe(true);
  });

  it('should register zcrystal_log_recent', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_log_recent')).toBe(true);
  });

  it('should register zcrystal_webhook_telegram', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_webhook_telegram')).toBe(true);
  });

  it('should register zcrystal_webhook_signal', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_webhook_signal')).toBe(true);
  });

  it('should register zcrystal_webhook_generic', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_webhook_generic')).toBe(true);
  });

  it('should register zcrystal_heartbeat_run', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_heartbeat_run')).toBe(true);
  });

  it('should register zcrystal_heartbeat_status', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_heartbeat_status')).toBe(true);
  });

  it('should register zcrystal_layers_exchange', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_layers_exchange')).toBe(true);
  });

  it('should register zcrystal_predict', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_predict')).toBe(true);
  });

  it('should register zcrystal_selfimproving_status', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    expect(mockApi.getTools().some((t: any) => t.name === 'zcrystal_selfimproving_status')).toBe(true);
  });
  ...
  it('should execute zcrystal_correction_add', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    const tool = mockApi.getTools().find((t: any) => t.name === 'zcrystal_correction_add');
  ...
  it('should execute zcrystal_session_get', async () => {
    const { registerProactiveTools } = await import('./proactive-tools.js');
    registerProactiveTools(mockApi, mockState);
    const tool = mockApi.getTools().find((t: any) => t.name === 'zcrystal_session_get');
